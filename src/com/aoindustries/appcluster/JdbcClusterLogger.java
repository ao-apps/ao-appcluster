...
public class JdbcClusterLogger implements AppClusterLogger {
    ...
    public JdbcClusterLogger(String name) throws AppClusterConfigurationException {
        try {
            Context ic = new InitialContext();
            Context envCtx = (Context)ic.lookup("java:comp/env");
            DataSource dataSource = (DataSource)envCtx.lookup(name);
            if(dataSource==null) throw new AppClusterConfigurationException(ApplicationResources.accessor.getMessage("JdbcClusterLogger.init.datasourceNotFound", name));
